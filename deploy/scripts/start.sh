...
# ---------- helpers ----------
gen_secret() {
  # 32 hex bytes from /dev/urandom — no openssl dependency
...
# ---------- first-run init (only if .env.ops doesn't exist) ----------
if [ ! -f "$ENV_OPS" ]; then
...
# Edit .env instead; changes are picked up on next ./start.sh.
